...
void H5Interface::writeMatrix(std::unordered_map<uint32_t, std::vector<matrixElement>*>* pGenomicDistanceMap, 
    ...
    int RANK = 1;
    // indptr
    H5::DataSpace* dataspaceIndptr = new H5::DataSpace(RANK, indptr);
    // create dataset
    ...
    // indices
    H5::DataSpace* dataspaceIndices = new H5::DataSpace(RANK, indices);
    // create dataset
    H5::DataSet* datasetIndices r = new H5::DataSet(file->createDataSet("/matrix/indices"), H5::PredType::NATIVE_INT32,
                                                *dataspaceIndices);
    delete dataspaceIndices;
    delete datasetIndices;

    // data
    H5::DataSpace* dataspaceData = new H5::DataSpace(RANK, data);
    // create dataset
